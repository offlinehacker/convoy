...
func doCreateNFSVolume(request *api.VolumeCreateRequest, optionsMap map[string]string) error {
...
func doMount(c *cli.Context) error {
	...
	case "vfs":
		return doMountNFS(request, optionsMap)
	case "efs":
		return nil
	default:
		return fmt.Errorf("unrecognized convoyDriver name specified")
...
func doMountNFS(request *api.VolumeMountRequest, optionsMap map[string]string) error {
	name, ok := optionsMap["name"]
	if !ok {
		return fmt.Errorf("no name option specified")
	}
